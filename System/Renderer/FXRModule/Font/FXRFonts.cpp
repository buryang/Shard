...
#include "Graphics/FXRModule/Font/FXRFonts.h"
...
namespace Shard::Renderer::FXR
{
    ...
    uint32_t FXRDrawText::AddFontStyle(const String& name, const Span<uint8_t>& bin)
    {
    ...
    void FXRDrawText::DrawExecuteFreeType(Render::RenderGraphBuilder& builder, const WString& wtext, const TextDrawParams& draw_params) {
    ...
    void FXRDrawText::DrawExecuteGlyphOutlines(Render::RenderGraph& graph, const WString& wtext, const TextDrawParams& draw_params) {
    ...
    void FXRDrawText::Draw(Render::RenderGraphBuilder& builder, const String& text, const TextDrawParams& draw_params)
    {
        const auto& wtext_temp = Utils::StringConvertHelper::StringToWString(text);
        Draw(builder, text, draw_params);
    }

    void FXRDrawText::Draw(Render::RenderGraphBuilder& builder, const WString& wtext, const TextDrawParams& draw_params)
    {
        PCHECK(!wtext.empty());
    ...
    void FXRDrawText::UpdateAtlas(float scale)
    {
        if (glyphs_neededLUT_.empty()) {
            return;
        }
