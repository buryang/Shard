...
namespace Shard::Render {
    ...
    IMPLEMENT_TYPE_LAYOUT_DEF(RenderShaderContent);

    bool RenderShaderType::Regist(Ptr shader_type)
    {
    ...
    RenderShaderType::RenderShaderType(const String& name, const String& file, const String& func, EShaderFrequency freq) :name_(name), file_name_(file), entry_func_(func), frequency_(freq)
    {
    }

    const uint32_t RenderShaderType::GetPermutationCount() const
    {
        return uint32_t();
    }

    void RenderShaderCompiledRepo::Serialize(Utils::FileArchive& archive) const
    {
    ...
    void RenderShaderCompiledRepo::UnSerialize(Utils::FileArchive& archive)
    {
        size_t sections_count{ 0 };
        archive << sections_count;
        for (auto n = 0; n < sections_count; ++n) {
            auto* section = new RenderShaderCompiledFileMap;
    ...
    void RenderShaderCompiledRepo::BeginCompile(const ShaderCompileEnv& compile_env) {
        ...
        for (auto iter = RenderShaderType::Begin(); iter != RenderShaderType::End(); ++iter) {
            ...
            else
            {
                ...
                {
                    shader_section = new RenderShaderCompiledFileMap;
...
#ifdef _WIN32
                    auto* compile_task = new RenderShaderDXCCompileWorker(job, shader_section);
#else
    ...
    void RenderShaderCompiledRepo::EndCompile() {
        //sync compile work
        RtGlobalCompileWorkManager::Instance().FinalizeAllJobs();
    }

    void RenderShaderCompiledRepo::Compile() {
    ...
    bool RenderShaderCompiledFileMap::IsReCompileNeeded()const {
    ...
    RenderShaderCompiledFileMap::Ptr RenderShaderCompiledRepo::FindOrCreateSection(const RenderShaderType::HashType hash_name)
    {
        ...
        auto* section = new RenderShaderCompiledFileMap();//todo
        shader_sections_.insert({section_name, section});
        return section;
    }

    RenderShaderCompiledRepo::~RenderShaderCompiledRepo()
    {
    ...
    RenderShader::SharedPtr RenderShaderCompiledRepo::ReadWait(HashType hash)const
    {
        auto ptr = Read(hash);
        if (ptr.get() == nullptr) {
    ...
    void RenderShaderCompiledRepo::Add(HashType hash, RenderShader::SharedPtr shader)
    {
        std::unique_lock<std::shared_mutex>(write_mutex_);
        shader_repo_.insert({hash, shader});
    }

    void RenderShaderCompiledRepo::Remove(HashType hash)
    {
        std::unique_lock<std::shared_mutex>(write_mutex_);
...
#ifdef _WIN32
    RenderShaderDXCCompileWorker::~RenderShaderDXCCompileWorker()
    {
    }

    void RenderShaderDXCCompileWorker::Compile()
    {
    ...
    void ShaderCompileJobIn::InitByShaderType(const RenderShaderType& shader_type, uint32_t permutation_id) {
    ...
    RenderShaderPipeline::SharedPtr Shard::Render::RtPipelineCompiledRepo::GetPipeline(const PipelineStateObjectDesc& desc)
    {
        RenderShaderPipeline::SharedPtr  pso(new RenderShaderPipeline(desc));
        return pso;
    }
