...
public:\
    void* operator new(size_type size){ \
        ...
        return g_rhi_allocator->allocate(size); \
    } \
    void operator delete(void* ptr, size_type size){ \
        g_rhi_allocator->deallocate(ptr, size); \
}
...
public:
    ...
    void operator delete[](void* ptr, size_type size) {\
        g_rhi_allocator->deallocate((uint8_t*)ptr, size);\
    }

namespace Shard::RHI
{
    //global memory alloc for rhi
    extern Shard::Utils::ScalablePoolAllocator<uint8_t>* g_rhi_allocator;

    template<typename T>
    FORCE_INLINE T* NewArray(size_type count) {
        return new(g_rhi_allocator->allocate(sizeof(T) * count), count)T; //todo
    }
    ...
    FORCE_INLINE void DeleteArray(T* ptr, size_type count) {
        ...
        g_rhi_allocator->deallocate((uint8_t*)ptr, sizeof(T) * count);
    }
