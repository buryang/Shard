...
#include "HAL/Vulkan/HALShaderLibraryVulkan.h"
...
namespace Shard::HAL::Vulkan {
    ...
    public:
        ...
        void Serialize() {
            ...
            auto ret = vkCreatEPipelineCache(GetGlobalDevice(), &create_info, g_host_alloc_vulkan, &rhi_cache_);
        ...
        ~PipelineStateObjectCacheVulkan() {
            if (rhi_cache_ != nullptr) {
                vkDestroyPipelineCache(GetGlobalDevice(), rhi_cache_, g_host_alloc_vulkan);
            }
        }
        HALPipelineStateObject* CreatEPipelineStateObject(const HALPipelineStateObjectInitializer& initializer) {
            return new HALPipelineStateObjectVulkan(initializer);
        }
    ...
    void HALPipelineStateObjectLibraryVulkan::Init()
    {
        pso_caches_.clear();
    ...
    HALPipelineStateObjectLibraryVulkan::~HALPipelineStateObjectLibraryVulkan()
    {
    ...
    HALPipelineStateObject* HALPipelineStateObjectLibraryVulkan::CreatEPipelineImpl(const HALPipelineStateObjectInitializer& initializer)
    {
    ...
    VkPipelineColorBlendStateCreateInfo* BlendStateVulkanCreator::Create(const HALBlendStateInitializer& initializer)
    {
        ...
        const auto trans_blend_factor = [](HALBlendStateInitializer::BlendAttachment::EBlendFactor factor) { return VkBlendFactor(); };
        const auto trans_blend_op = [](HALBlendStateInitializer::BlendAttachment::EBlendOperation op) { return VkBlendOp(); };
        const auto trans_color_wmask = [](HALBlendStateInitializer::BlendAttachment::EBlendColorMask mask) { return VkColorComponentFlags(); };
    ...
    VkPipelineRasterizationStateCreateInfo* RasterizationStateVulkanCreator::Create(const HALRasterizationStateInitializer& initializer)
    {
    ...
    VkPipelineDepthStencilStateCreateInfo* DepthStencilStateVulkanCreator::Create(const HALDepthStencilStateInitializer& initializer)
    {
        auto* ds_state = new VkPipelineDepthStencilStateCreateInfo;
        const auto trans_compare_op = [](HALDepthStencilStateInitializer::EDepthCompareOp op) { return VkCompareOp(); };
        const auto trans_stentil_op = [](HALDepthStencilStateInitializer::EStencilOp op) { return VkStencilOpState(); };
