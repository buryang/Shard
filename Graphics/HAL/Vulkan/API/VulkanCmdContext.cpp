...
#include "HAL/Vulkan/API/VulkanRayTraceKHR.h"
#include "HAL/Vulkan/API/VulkanResources.h"

namespace Shard
{
    /*copy from falcor*/
    static inline VkEPipelineStageFlags ResourceStateToStageFlags(EResourceState state, bool is_src)
    {
        switch (state)
        {
    ...
    VulkanCmdPool::VulkanCmdPool(VkCommandPoolCreateFlags flags, uint32_t family_index):family_index_(family_index)
    {
        ...
        auto ret = vkCreateCommandPool(GetGlobalDevice(), &create_info, g_host_alloc_vulkan, &pool_);
    ...
    VulkanCmdPool::~VulkanCmdPool()
    {
        ...
        {
            vkDestroyCommandPool(GetGlobalDevice(), handle_, g_host_alloc_vulkan);
        }
    }
    ...
    VulkanCmdBuffer::SharedPtr VulkanCmdBuffer::Create(VulkanCmdPool::SharedPtr cmd_pool)
    {
        ...
        auto ret = vkCreateFence(GetGlobalDevice(), &fence_info, g_host_alloc_vulkan, &cmd_ptr->fence_);
    ...
    VulkanCmdBuffer::~VulkanCmdBuffer()
    {
        ...
        {
            Reset();
            vkDestroyFence(GetGlobalDevice(), fence_, g_host_alloc_vulkan);
        }
    }
    ...
    void VulkanCmdBuffer::TraceRay(Map<uint32_t, VulkanRayTraceBindTable>& ray_binds, const glm::ufloat3& dims)
    {
        ...
        vkCmdTraceRaysKHR(buffer_, ray_binds[static_cast<uint32_t>(VulkanRayTraceBindTable::Type::RGEN)],
            ...
            ray_binds[static_cast<uint32_t>(VulkanRayTraceBindTable::Type::HALT)],
            ray_binds[static_cast<uint32_t>(VulkanRayTraceBindTable::Type::RCALL)],
            dims.x, dims.y, dims.z);
        */
    }

    void VulkanCmdBuffer::Clear(VulkanBuffer& buffer, uint32_t value, VkDeviceSize offset, VkDeviceSize size)
    {
        vkCmdFillBuffer(handle_, buffer.Get(), offset, size, value);
    }

    void VulkanCmdBuffer::Clear(VulkanImage& image, VkClearValue value, const VkImageSubresourceRange& region)
    {
        if (region.aspectMask & VK_IMAGE_ASPECT_COLOR_BIT)
        {
            const auto& color_value = value.color;
            vkCmdClearColorImage(handle_, image.Get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &color_value, 1, &region);
        }
        else if (region.aspectMask & (VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT))
        {
            const auto& depth_value = value.depthStencil;
            vkCmdClearDepthStencilImage(handle_, image.Get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &depth_value, 1, &region);
        }
        else
        {
            LOG(ERROR) << "invalid region aspect mask";
        }
    }
    ...
    void VulkanCmdBuffer::Barrier(const HAL::VulkanTransitionInfo& barrier_info)
    {
