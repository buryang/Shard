...
namespace Shard
{
    void MakeImageCreateInfo(VkImageCreateInfo& image_info, VkImageCreateFlags flags, VkFormat format, uint32_t layers, uin32_t mips)
    {
    ...
    void MakeImageViewCreateInfo(VkImageViewCreateInfo& view_info, VkImageViewCreateFlags flags, VkImage image, VkImageViewType view_type)
    {
    ...
    void MakeSamplerCreateInfo(VkSamplerCreateInfo& sample_info, VkSamplerCreateFlags flags, VkFilter mag_filter, VkFilter min_filter,
    ...
    VulkanImage::VulkanImage(VulkanDevice* device, VulkanBuffer::SharedPtr buffer):device_(device)
    {
        //todo
        ReadyForTransmit();
    ...
    VulkanImage::VulkanImage(VulkanDevice* device, const VkImageCreateInfo& create_info):device_(device)
    {
        auto ret = vkCreateImage(device->Get(), &create_info, g_host_alloc_vulkan, &handle_);
    ...
    VulkanImage::~VulkanImage()
    {
        ...
        {
            vkDestroyImage(device_->Get(), handle_, g_host_alloc_vulkan);
            //vmaFreeMemory(nullptr, vma_data_.allocation_);
        }
    }

    void VulkanImage::Clear(VulkanCmdBuffer::SharedPtr cmd_buffer, )
    {
    ...
    VulkanImageView::VulkanImageView(VulkanImage* image, VkImageViewType view_type, VkFormat format, uint32_t mip_level,
                                        ...
                                        
    {
        ...
        auto ret = vkCreateImageView(image->graph_->GetDevice()->Get(), view_info, g_host_alloc_vulkan, &handle_);
    ...
    VulkanImageView::~VulkanImageView()
    {
        if (VK_NULL_HANDLE != handle_)
        {
            vkDestroyImageView(image_->GetDevice()->Get(), handle_, g_host_alloc_vulkan);
        }
    }
    ...
    VulkanSampler::~VulkanSampler()
    {
        if (VK_NULL_HANDLE != handle_)
        {
            vkDestroySampler(nullptr, handle_, g_host_alloc_vulkan);
        }
    }
    ...
    VulkanBuffer::VulkanBuffer(VulkanDevice* device, const VkBufferCreateInfo& create_info):prop_info_(create_info)
    {
        auto ret = vkCreateBuffer(device->Get(), &create_info, g_host_alloc_vulkan, &handle_);
                                        ...
                                        uint32_t offset, uint32_t range):buffer_(buffer)
    {
        ...
        auto ret = vkCreateBufferView(GetGlobalDevice(), &view_info, g_host_alloc_vulkan, &handle_);
    }
    ...
    VulkanBufferView::~VulkanBufferView()
    {
        if (VK_NULL_HANDLE != handle_)
        {
            vkDestroyBufferView(GetGlobalDevice(), handle_, g_host_alloc_vulkan);
        }
    }
