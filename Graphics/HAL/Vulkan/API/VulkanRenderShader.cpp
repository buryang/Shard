...
namespace Shard {
    ...
    VulkanShaderModule::VulkanShaderModule(VulkanDevice* device, const Span<char>&code )
    {
        const auto& create_info = MakeShaderModuleCreateInfo(code);
        Init(device, create_info);
    }

    void VulkanShaderModule::Init(VulkanDevice* device, const VkShaderModuleCreateInfo& create_info)
    {
        assert(handle_ == VK_NULL_HANDLE);
        vkCreateShaderModule(device->Get(), &create_info, g_host_alloc_vulkan, &handle_);
    }

    VulkanShaderModule::VulkanShaderModule(VulkanDevice* device, const VkShaderModuleCreateInfo& create_info):device_(device)
    {
        vkCreateShaderModule(device->Get(), &create_info, g_host_alloc_vulkan, &handle_);
    }
    ...
    VulkanShaderModule::~VulkanShaderModule()
    {
        ...
        {
            vkDestroyShaderModule(device_->Get(), handle_, g_host_alloc_vulkan);
        }
    }
