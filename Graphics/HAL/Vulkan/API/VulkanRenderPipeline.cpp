...
namespace Shard
{
    ...
    VulkanRenderPipeline* VulkanRenderPipeline::Create(VulkanDevice* device, const VulkanRenderPipeline::Desc& desc)
    {
        VulkanRenderPipeline* pipe_handle;
        if (VulkanRenderPipeline::EPipeType::eCompute == desc.pipe_type_)
        {
            pipe_handle.reset(new VulkanComputEPipeline(device, desc));
        }
        else if (EPipeType::eGFX == desc.pipe_type_)
        {
    ...
    VulkanRenderPipeline::VulkanRenderPipeline(VulkanDevice* device, const Desc& desc, EPipeType pipe_type) : device_(device), pipe_type_(pipe_type)
    {
        ...
        auto ret = vkCreatEPipelineLayout(device_->Get(), &layout_info, g_host_alloc_vulkan, &layout_);
        if (VK_SUCCESS != ret)
        {
    ...
    VulkanRenderPipeline::~VulkanRenderPipeline()
    {
        ...
        {
            vkDestroyPipeline(device_->Get(), handle_, g_host_alloc_vulkan);
        }

        if (VK_NULL_HANDLE != layout_)
        {
            vkDestroyPipelineLayout(device_->Get(), layout_, g_host_alloc_vulkan);
        }
        ...
        for (auto& ds_layout : ds_layouts_)
        {
            ...
            {
                vkDestroyDescriptorSetLayout(device_->Get(), ds_layout, g_host_alloc_vulkan);
            }
        }
    }
    ...
    void VulkanRenderPipeline::InitialDescSetLayouts(const RootSignature& root)
    {
        ...
        for (auto n = 0; n < root.GetNumDescriptorSets(); ++n)
        {
            ...
            vkCreateDescriptorSetLayout(device_->Get(), &set_create_info, g_host_alloc_vulkan, &set_handle);
    ...
    VulkanComputEPipeline::VulkanComputEPipeline(VulkanDevice* device, const VulkanRenderPipeline::Desc& param) : VulkanRenderPipeline(device, param, EPipeType::eCompute)
    {
        ...
        auto ret = vkCreateComputEPipelines(device_->Get(), device_->GetPipelineCache(), 1, &pipeline_info, g_host_alloc_vulkan, &handle_);
        if (VK_SUCCESS != ret)
        {
    ...
    VulkanGraphicsPipeline::VulkanGraphicsPipeline(VulkanDevice* device, const VulkanRenderPipeline::Desc& param) : VulkanRenderPipeline(device, EPipeType::eGFX)
    {
        ...
        auto ret = vkCreateGFXPipelines(device_->Get(), param.pipeline_cache_, 1, &pipeline_info, g_host_alloc_vulkan, &handle_);
        PCHECK(VK_SUCCESS != ret) << "graphics pipeline contruct failed";
    }

    VulkanShaderModule* VulkanGraphicsPipeline::GetShader(VulkanShaderModule::EType shader_type)
    {
    ...
    VulkanRayTracingPipeline::VulkanRayTracingPipeline(VulkanDevice* device, const VulkanRenderPipeline::Desc& param) : VulkanRayTracingPipeline(device, EPipeType::eRayTrace)
    {
