...
namespace Shard
{
    ...
    VulkanDescriptorPool::~VulkanDescriptorPool()
    {
        ...
        {
            vkDestroyDescriptorPool(GetGlobalDevice(), pool, g_host_alloc_vulkan);
        }
        available_.clear();
    ...
    void VulkanDescriptorPool::UpdateCurrPool()
    {
        if (available_.empty())
        {
            ...
            auto ret = vkCreateDescriptorPool(GetGlobalDevice(), &pool_info, g_host_alloc_vulkan, &pool);
    ...
    VulkanDescriptorSet::VulkanDescriptorSet(VulkanDescriptorPool::SharedPtr pool, const VulkanDescriptorSetLayoutDesc& set_desc):name_(set_desc.name_)
    {
        ...
        auto ret = vkCreateDescriptorSetLayout(GetGlobalDevice(), &set_desc.ToVulkan(), g_host_alloc_vulkan, &layout_);
    ...
    VulkanDescriptorSet::~VulkanDescriptorSet() {
        if (layout_ != VK_NULL_HANDLE) {
            vkDestroyDescriptorSetLayout(GetGlobalDevice(), layout_, g_host_alloc_vulkan);
