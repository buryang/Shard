#include "HAL/VulkanWindowSystem.h"
#include "HAL/VulkanHAL.h"
...
namespace Shard
{
    ...
    VulkanWindowSystemImpl::VulkanWindowSystemImpl(VulkanInstance* instance, VulkanDevice* device, GLFWwindow* window) : instance_(instance), device_(device), window_(window)
    {
        //first create a surface
        auto ret = glfwCreateWindowSurface(instance_->Get(), window, g_host_alloc_vulkan, &surface_);
        if (VK_SUCCESS != ret)
        {
        ...
        {
            vkDestroySurfaceKHR(instance_->Get(), surface_, g_host_alloc_vulkan);
        ...
        ret = vkCreateSwapchainKHR(device->Get(), &swap_info, g_host_alloc_vulkan, &swap_chain_);
        if (VK_SUCCESS != ret)
        {
            vkDestroySurfaceKHR(instance_->Get(), surface_, g_host_alloc_vulkan);
    ...
    VulkanWindowSystemImpl::~VulkanWindowSystemImpl()
    {
        ...
        {
            device_->WaitIdle();
            vkDestroySwapchainKHR(device_->Get(), swap_chain_, g_host_alloc_vulkan);
        }
    ...
    void VulkanWindowSystemImpl::OnWindowChange(GLFWwindow* window)
    {
        ...
        if (VK_NULL_HANDLE != swap_chain_)
        {
            vkDestroySwapchainKHR(device_->Get(), nullptr, g_host_alloc_vulkan);
        }
        auto ret = glfwCreateWindowSurface(instance_->Get(), window, g_host_alloc_vulkan, &surface_);
        if (VK_SUCCESS != ret)
        {
    ...
    void VulkanWindowSystemImpl::InitFrameWorkLoads()
    {
        if (VK_NULL_HANDLE != swap_chain_)
        {
            ...
            { 
                ...
                vkCreateImageView(device_->Get(), &view_info, g_host_alloc_vulkan, &(frame_workloads_[n].image_view_));
                VkFenceCreateInfo fence_info{};
                fence_info.sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO;
                fence_info.pNext = VK_NULL_HANDLE;
                fence_info.flags = VK_FENCE_CREATE_SIGNALED_BIT;
                vkCreateFence(device_->Get(), &fence_info, g_host_alloc_vulkan, &(frame_workloads_[n].fence_));
            }
            ...
            for (auto& acquire : acquires_)
            {
                ...
                vkCreateSemaphore(device_->Get(), &semp_info, g_host_alloc_vulkan, &acquire);
            }
        }
    }
    void VulkanWindowSystemImpl::ResetFrameWorkLoads()
    {
        ...
        for (auto& frame : frame_workloads_)
        {
            ...
            {
                vkDestroyImage(device_->Get(), frame.image_, g_host_alloc_vulkan);
            }

            if (frame.image_view_ != VK_NULL_HANDLE)
            {
                vkDestroyImageView(device_->Get(), frame.image_view_, g_host_alloc_vulkan);
            }

            if (frame.fence_ != VK_NULL_HANDLE)
            {
                vkDestroyFence(device_->Get(), frame.fence_, g_host_alloc_vulkan);
            }
        }
        ...
        for (auto& acquire : acquires_)
        {
            ...
            {
                vkDestroySemaphore(device_->Get(), acquire, g_host_alloc_vulkan);
            }
        }
        acquires_.clear();
    }
