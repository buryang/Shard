...
bool IntersectBox(float3 ray_orign, float3 ray_dir, float3 box_center, float3 box_half, float t_min, float t_max, inout float fly_time)
{
    ...
    float max_min = max3(min_time.x, min_time.y, min_time.z, t_min);
    float min_max = min3(max_time.x, max_time.y, max_time.z, t_max);
    fly_time = max_min;
    
    return max_min < min_max;
}

//https://gpfault.net/posts/aabb-tricks.html, same alogirthm as above(slab method) but with min/max inpu
bool IntersectBox(float3 ray_origin, float3 ray_dir, float3 box_min, float3 box_max, float t_min, float t_max, inout float fly_time)
{
    float3 inv_dir = rcp(ray_dir);
    float3 t_minwalls = (box_min - ray_origin) * inv_dir;
    float3 t_maxwalls = (box_max - ray_origin) * inv_dir;
    ...
    float4 t_ranges_starts = float4(min(t_minwalls, t_maxwalls), t_min);
    float4 t_ranges_ends = float4(max(t_minwalls, t_maxwalls), t_max);
    
    float largest_start = max4(t_ranges_starts);
    float smallest_end = min4(t_ranges_ends);
    
    fly_time = largest_start;
...
//hardware-ray tracing inline functions
struct HRM
{
